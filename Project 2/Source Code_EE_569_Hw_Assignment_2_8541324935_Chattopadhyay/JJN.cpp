   ...
   for(length = 0; length<Size; length++)
   {
       if(length%2 == 0)
       {
        for(breadth = 0; breadth<Size; breadth++)
        {
            ...
            if (breadth+1 < Size)
            {
                temp[length][breadth+1]=temp[length][breadth+1]+error*7/48;
            }
            if (length+1 < Size && breadth+1 < Size)
            {
                temp[length+1][breadth+1]=temp[length+1][breadth+1]+error*5/48;
            }
            if (length+2 < Size && breadth+1 < Size)
            {
                temp[length+2][breadth+1]=temp[length+2][breadth+1]+error*3/48;
            }
            if (breadth+2 < Size)
            {
                temp[length][breadth+2]=temp[length][breadth+2]+error*5/48;
            }
            if (length+1 < Size && breadth+2 < Size)
            {
                temp[length+1][breadth+2]=temp[length+1][breadth+2]+error*3/48;
            }
            if (length+2 < Size && breadth+2 < Size)
            {
                temp[length+2][breadth+2]=temp[length+2][breadth+2]+error*1/48;
            }
            if (length+1 < Size)
            {
                temp[length+1][breadth]=temp[length+1][breadth]+error*7/48;
            }
            if (length+2 < Size)
            {
                temp[length+2][breadth]=temp[length+2][breadth]+error*5/48;
            }
