...
int main(int argc, char *argv[]){	// Define file pointer and variables	FILE *file;	int BytesPerPixel;	int Size = 512;	// Check for proper syntax	if (argc < 3){		cout << "Syntax Error - Incorrect Parameter Usage:" << endl;		cout << "program_name input_image.raw output_image.raw [BytesPerPixel = 1] [Size = 512]" << endl;		return 0;	}	// Check if image is grayscale or color	if (argc < 4)
    ...
    for(length = 0; length<Size; length++)
    {
      if(length%2 == 0)
      {
        for(breadth = 0; breadth<Size; breadth++)
        {
            ...
            if (breadth+1 < Size)
            {
                temp[length][breadth+1]=temp[length][breadth+1]+error*8/42;
            }
            if (length+1 < Size && breadth+1 < Size)
            {
                temp[length+1][breadth+1]=temp[length+1][breadth+1]+error*4/42;
            }
            if (length+2 < Size && breadth+1 < Size)
            {
                temp[length+2][breadth+1]=temp[length+2][breadth+1]+error*2/42;
            }
            if (breadth+2 < Size)
            {
                temp[length][breadth+2]=temp[length][breadth+2]+error*4/42;
            }
            if (length+1 < Size && breadth+2 < Size)
            {
                temp[length+1][breadth+2]=temp[length+1][breadth+2]+error*2/42;
            }
            if (length+2 < Size && breadth+2 < Size)
            {
                temp[length+2][breadth+2]=temp[length+2][breadth+2]+error*1/42;
            }
            if (length+1 < Size)
            {
                temp[length+1][breadth]=temp[length+1][breadth]+error*8/42;
            }
            if (length+2 < Size)
            {
                temp[length+2][breadth]=temp[length+2][breadth]+error*4/42;
            }
            if (length+1 < Size && breadth-1 >= 0)
            {
                temp[length+1][breadth-1]=temp[length+1][breadth-1]+error*4/42;
            }
            if (length+2 < Size && breadth-1 >= 0)
            {
                temp[length+2][breadth-1]=temp[length+2][breadth-1]+error*2/42;
            }
    ...
    {
        for (breadth = Size-1; breadth>=0; breadth--)
        {
             if (temp[length][breadth]>=0.5)
            {
                error = temp[length][breadth]-1;
