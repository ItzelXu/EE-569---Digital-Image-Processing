    ...
    for(length = 0; length<Size; length++)
    {
        if(length%2 == 0)
        {
           for(breadth = 0; breadth<Size; breadth++)
           {
               ...
               if (breadth+1 < Size)
               {
                  temp[length][breadth+1]=temp[length][breadth+1]+error*7/16;
               }
               if (breadth-1 >= 0 && length+1 < Size)
               {
                  temp[length+1][breadth-1]=temp[length+1][breadth-1]+error*3/16;
               }
               if (length+1 < Size)
               {
                  temp[length+1][breadth]=temp[length+1][breadth]+error*5/16;
               }
               if (length+1 < Size && breadth+1 < Size)
               {
                  temp[length+1][breadth+1]=temp[length+1][breadth+1]+error*1/16;
               }
           }
        }
        else
